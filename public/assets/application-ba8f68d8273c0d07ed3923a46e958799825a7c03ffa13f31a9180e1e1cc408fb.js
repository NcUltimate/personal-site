...
(function() {
  ...
  Background = {
    ...
    initGrid: function() {
      var lines;
      lines = this.svg.selectAll('grid-line').data(this.gridPoints()).enter().append('line');
      return lines.attr('x1', function(d) {
        ...
        return d[3];
      }).attr('stroke', '#e5e5e5').classed('grid-line', true);
    },
    gridPoints: function() {
      var endpoints, i, j, perc, perc2, ref, size;
      endpoints = [];
      size = 20;
      for (j = i = 0, ref = size; 0 <= ref ? i <= ref : i >= ref; j = 0 <= ref ? ++i : --i) {
        perc = (100 / size) * j + '%';
        perc2 = (50 / size) * j + '%';
...
(function() {
  ...
  $(function() {
    return Cinemagraph1.initialize('#forest');
  });
  ...
  Cinemagraph1 = {
    ...
    initialize: function(svg) {
      var $fireflies, world1;
      this.svgId = svg;
      this.svg = d3.select(svg);
      this.addFireflies();
      $fireflies = $('.firefly');
      world1 = this;
      $.each($fireflies, function(idx, f) {
        var delay, duration, opacity, points, t1;
      ...
      return this.initAmbience($fireflies);
    },
    initAmbience: function($fireflies) {
      var period, t1, t2, t3;
      period = 12;
      t1 = new TimelineMax({
        repeat: -1
      ...
      return t3.to('', period / 4, {});
    },
    addFireflies: function() {
      var circles, data, delay, duration, i, k, r, x, y;
      data = [];
      for (k = i = 0; i <= 100; k = ++i) {
      ...
      return circles.attr('cx', function(d) {
        ...
        return d.r;
      }).attr('fill', 'url(#firefly_light)').classed('firefly', true).style('animation', function(d) {
        return 'glow ' + d.duration + 's ease-in-out ' + d.delay + 's infinite';
      });
    },
    randomPoints: function() {
      var i, k, npoints, points, radius, ref, x, y;
      radius = 100;
      npoints = Math.floor(Math.random() * 5 + 6);
      points = [];
      for (k = i = 0, ref = npoints; 0 <= ref ? i <= ref : i >= ref; k = 0 <= ref ? ++i : --i) {
...
(function() {
  ...
  $(function() {
    return Cinemagraph2.initialize('#mountain');
  });
  ...
  Cinemagraph2 = {
    ...
    initialize: function(svg) {
      ...
      return t1.addCallback(this.updateAll, 0.3, [this]);
    },
    updateAll: function(cinemagraph2) {
      cinemagraph2.updateSnowFlakes(cinemagraph2);
      return cinemagraph2.updateSmoke(cinemagraph2);
    },
    updateSnowFlakes: function(cinemagraph2) {
      var top, w;
      w = $(cinemagraph2.svgId).parent().width();
      top = cinemagraph2.svg[0][0].getBoundingClientRect().top;
      cinemagraph2.addSnowFlakes(10, w, -10);
      return $.each(d3.selectAll('.snowflake')[0], function(idx, s) {
        if (s.getBoundingClientRect().top - top > 300) {
          return s.remove();
        }
      });
    },
    updateSmoke: function(cinemagraph2) {
      var top, w;
      w = $(cinemagraph2.svgId).parent().width();
      top = cinemagraph2.svg[0][0].getBoundingClientRect().top;
      cinemagraph2.addSmoke(5);
      return $.each(d3.selectAll('.smoke')[0], function(idx, s) {
        if ($(s).css('opacity') === '0') {
          return $(s).remove();
        }
      });
    },
    addSnowFlakes: function(num, w, h) {
      var circles, data, i, k, opacity, radius, ref, x, y;
      data = [];
      for (k = i = 0, ref = num; 0 <= ref ? i <= ref : i >= ref; k = 0 <= ref ? ++i : --i) {
      ...
      circles = this.svg.selectAll('snowflake').data(data).enter().append('circle');
      circles.attr('cx', function(d) {
        ...
        return d.opacity;
      }).classed('snowflake', true).classed('unfallen', true);
      return this.initSnowPaths();
    },
    addSmoke: function(num) {
      var circles, data, i, k, opacity, radius, ref;
      data = [];
      for (k = i = 0, ref = num; 0 <= ref ? i <= ref : i >= ref; k = 0 <= ref ? ++i : --i) {
      ...
      circles = this.svg.selectAll('smoke').data(data).enter().append('circle');
      circles.attr('cx', $(this.svgId).parent().width() - 160).attr('cy', 215).attr('fill', 'url(#smoke)').attr('r', function(d) {
        return d.radius;
      }).attr('opacity', function(d) {
        return d.opacity;
      }).classed('smoke', true).classed('unrisen', true);
      return this.initSmokePaths();
    },
    initSmokePaths: function() {
      var cinemagraph2;
      cinemagraph2 = this;
      return $.each($('.smoke.unrisen'), function(idx, s) {
        var delay, duration, t1, x, y;
    ...
    initSnowPaths: function() {
      var cinemagraph2;
      cinemagraph2 = this;
      return $.each($('.snowflake.unfallen'), function(idx, s) {
        var duration, t1;
    ...
    generateSnowPath: function() {
      var dist, i, k, npoints, points, ref, rx, x, y;
      x = 0;
      y = 0;
      npoints = 5;
      points = [
...
(function() {
  ...
  Embroidery = {
    ...
    animate: function() {
      return $.each($('.stripe'), function(idx, s) {
        return TweenLite.from(s, 0.6, {
          scale: 0,
          delay: idx % 3 * 0.1 + 0.6,
          x: idx < 3 ? 200 : 0
        });
    ...
    initStripes: function() {
      var lines;
      lines = this.svg.selectAll('stripe').data([['5%', '40%', '30%', '40%'], ['10%', '50%', '31%', '50%'], ['15%', '60%', '32%', '60%'], ['70%', '40%', '95%', '40%'], ['69%', '50%', '90%', '50%'], ['68%', '60%', '85%', '60%']]).enter().append('g').classed('stripe', true);
      ...
      return lines.append('circle').attr('cx', function(d, i) {
        return d[2 * Math.floor(i / 3)];
      }).attr('cy', function(d, i) {
        return d[2 * Math.floor(i / 3) + 1];
      }).attr('r', 2).attr('fill', '#555');
    }
  };
...
(function() {
  ...
  Menu = {
    ...
    initialize: function() {
      this.svg = d3.select('#menu').attr('width', this.menuWidth).attr('height', this.menuWidth);
      this.menus = $.map(this.menuNames, function(m, idx) {
        return [[m, idx]];
      });
      this.initMenuCircle();
    ...
    animate: function() {
      ...
      this.pulse = new TimelineMax({
        repeat: -1
      });
      return this.pulse.staggerTo($('.menu-button'), 0.2, {
        yoyo: true,
        repeat: 1,
        scale: 0.7,
        delay: 8
    ...
    initMenuTitles: function() {
      var text, textPath;
      text = this.svg.selectAll('textpath').data([['My Work', '#menupath1'], ['My Self', '#menupath2']]).enter().append('text');
      text.attr('fill', '#555').attr('font-size', '22').classed('menu-title', true);
      return textPath = text.append('textPath').attr('xlink:href', function(d) {
        return d[1];
      }).text(function(d) {
        return d[0];
      });
    },
    initMenuCircle: function() {
      var menuCircle;
      menuCircle = this.svg.selectAll('menu-circle').data([[this.menuRadius + 5, 1], [this.menuRadius, 2]]).enter().append('circle');
      return menuCircle.attr('cx', this.menuWidth / 2).attr('cy', this.menuWidth / 2).attr('r', function(d) {
        return d[0];
      }).attr('fill', '#eee').attr('stroke-width', function(d) {
        return d[1];
      }).attr('stroke', '#ccc').classed('menu-circle', true);
    },
    initSiteName: function() {
      var siteName, subtitle;
      siteName = this.svg.selectAll('sitename').data(["JoeKle.men"]).enter().append('text');
      siteName.text(function(d) {
        return d;
      }).attr('fill', 'black').attr('x', this.menuWidth / 2 - 65).attr('y', this.menuWidth / 2).style('font-size', '26px').classed('sitename', true);
      subtitle = this.svg.selectAll('subtitle').data(["UI/UX Developer"]).enter().append('text');
      return subtitle.text(function(d) {
        return d;
      }).attr('fill', '#555').attr('x', this.menuWidth / 2 - 65).attr('y', this.menuWidth / 2 + 20).style('font-size', '16px').classed('sitename', true);
    },
    initMenuButtons: function() {
      var buttons, rad, site;
      site = this;
      buttons = this.svg.selectAll('menu-button').data(this.menus).enter().append('g').classed('menu-button', true).on('mouseover', function(e) {
        var c;
        d3.selectAll('.menu-circle').transition().ease('cubic-out').attr('stroke', '#555');
        c = d3.select(this).select('circle');
        c.transition().ease('cubic-out').attr('r', site.buttonRadius + 10).attr('stroke-width', 3);
        return site.pulse.seek(0);
      }).on('mouseout', function(e) {
        var c;
        d3.selectAll('.menu-circle').transition().ease('cubic-out').attr('stroke', '#ccc');
        c = d3.select(this).select('circle');
        c.transition().ease('cubic-out').attr('r', site.buttonRadius).attr('stroke-width', 1);
        return site.pulse.play();
      });
      rad = this.buttonRadius;
      buttons.append('circle').attr('cx', function(d) {
        return site.buttonPosition(d[1])[0];
      }).attr('cy', function(d) {
        return site.buttonPosition(d[1])[1];
      }).attr('r', rad).attr('fill', '#eee').attr('stroke', '#555');
      return buttons.append('text').attr('x', function(d) {
        return site.textPosition(site, d[1])[0];
      }).attr('y', function(d) {
        return site.textPosition(site, d[1])[1];
      }).attr('fill', '#333').text(function(d) {
        return d[0];
      });
    },
    buttonPosition: function(idx) {
      var angle, cx, cy, ridx, start, x, y;
      ridx = idx % (this.menus.length / 2) + 1;
      angle = -180 / (this.menus.length / 2 + 1);
      start = ridx * angle + 90;
      start += idx < this.menus.length / 2 ? 180 : 0;
      cx = this.menuWidth / 2;
      cy = this.menuWidth / 2;
      x = cx + Math.cos(this.toRad(start)) * this.menuRadius;
      y = cy + Math.sin(this.toRad(start)) * this.menuRadius;
      return [x, y];
    },
    textPosition: function(site, idx) {
      var bpos, x, y;
...
(function() {
  ...
  Main = {
    initialize: function() {
      ...
      return this.Embroidery.initialize();
    }
  };
  ...
  $(function() {
    if ($(window).width() <= 480) {
      setTimeout(function() {
        return $('.site-menu').addClass('hidden-left');
      }, 500);
    }
    $(document).on('click', '.hamburger', function(e) {
      return $('.site-menu').toggleClass('hidden-left');
    });
    return $(window).on('resize', function(e) {
      if ($(window).width() <= 480) {
        return $('.site-menu').addClass('hidden-left');
      } else {
        return $('.site-menu').removeClass('hidden-left');
      }
    });
  });
